(function () {
    ...
    var listarUrl = document.querySelector('input[name="tipo"]')?.value;
    if (!listarUrl) return;

    var tipoMap = {
        'general': '/notas/generales/listar',
        'cobranza': '/notas/cobranza/listar',
        'tributaria': '/notas/tributaria/listar',
        'laboral': '/notas/laboral/listar',
    };

    var tabla = new DataTable('#tablaNotas', {
        ajax: tipoMap[listarUrl] || '/notas/generales/listar',
        responsive: true,
        order: [[1, 'desc']],
        pageLength: 25,
    ...
    document.querySelector('[data-bs-target="#notaModal"]').addEventListener('click', function () {
        limpiarForm();
        modalTitle.textContent = 'Nueva Nota';
        form.dataset.mode = 'crear';
    });

    document.querySelector('#tablaNotas tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.editar-nota');
        if (btn) {
    ...
    document.querySelector('#tablaNotas tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.eliminar-nota');
        if (!btn) return;
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/notas/actualizar/' + form.dataset.editId : '/notas/guardar';
    ...
    function llenarForm(data) {
        ...
        document.getElementById('f_nota').value = data.nota || '';
    }
