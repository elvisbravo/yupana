(function () {
    ...
    var tabla = new DataTable('#tablaServicios', {
        ajax: '/servicios/contratados/listar',
        responsive: true,
        order: [[0, 'asc']],
        pageLength: 25,
    ...
    document.querySelector('[data-bs-target="#servicioModal"]').addEventListener('click', function () {
        limpiarForm();
    ...
    document.querySelector('#tablaServicios tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.editar-servicio');
        if (btn) {
            var id = btn.getAttribute('data-id');
            fetch('/servicios/contratados/obtener/' + id)
    ...
    document.querySelector('#tablaServicios tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.eliminar-servicio');
        if (!btn) return;
        if (!confirm('¿Eliminar este servicio contratado?')) return;
        var id = btn.getAttribute('data-id');
        fetch('/servicios/contratados/eliminar/' + id)
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/servicios/contratados/actualizar/' + form.dataset.editId : '/servicios/contratados/guardar';
    ...
    function llenarForm(data) {
        ...
        document.getElementById('f_tipo').value = data.tipo_servicio_id || '';
