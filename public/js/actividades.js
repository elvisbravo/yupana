(function () {
    ...
    var tabla = new DataTable('#tablaActividades', {
        ajax: '/clientes/actividades/listar',
        responsive: true,
        order: [[0, 'asc']],
        pageLength: 25,
    ...
    document.querySelector('[data-bs-target="#actividadModal"]').addEventListener('click', function () {
        limpiarForm();
        var hoy = new Date().toISOString().slice(0, 10);
        document.getElementById('f_fecha_inicio').value = hoy;
        modalTitle.textContent = 'Asignar Actividad';
        form.dataset.mode = 'crear';
    });

    document.querySelector('#tablaActividades tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.editar-actividad');
        if (btn) {
            var ids = btn.getAttribute('data-id').split('-');
            fetch('/clientes/actividades/obtener?cliente_id=' + ids[0] + '&actividad_id=' + ids[1])
                ...
                .then(function (data) {
                    ...
                    form.dataset.editClienteId = ids[0];
                    form.dataset.editActividadId = ids[1];
                    bsModal.show();
                });
        }
    });
    ...
    document.querySelector('#tablaActividades tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.eliminar-actividad');
        if (!btn) return;
        if (!confirm('¿Eliminar esta asignación?')) return;
        var ids = btn.getAttribute('data-id').split('-');
        fetch('/clientes/actividades/eliminar?cliente_id=' + ids[0] + '&actividad_id=' + ids[1])
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/clientes/actividades/actualizar' : '/clientes/actividades/guardar';
        var fd = new FormData(form);
        if (mode === 'editar') {
            fd.set('cliente_id', form.dataset.editClienteId);
            fd.set('actividad_id', form.dataset.editActividadId);
        }
    ...
    function llenarForm(data) {
        ...
        document.getElementById('f_actividad').value = data.actividad_id || '';
        document.getElementById('f_fecha_inicio').value = data.fecha_inicio || '';
        document.getElementById('f_principal').checked = data.es_principal == 1;
    }

    function limpiarForm() {
        form.reset(); alertBox.innerHTML = ''; btnGuardar.disabled = false; spinner.classList.add('d-none');
        delete form.dataset.mode; delete form.dataset.editClienteId; delete form.dataset.editActividadId;
    }
