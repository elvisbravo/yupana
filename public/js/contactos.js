(function () {
    ...
    var tabla = new DataTable('#tablaContactos', {
        ajax: '/clientes/contactos/listar',
        responsive: true,
        order: [[0, 'asc']],
        pageLength: 25,
    ...
    document.querySelector('[data-bs-target="#contactoModal"]').addEventListener('click', function () {
        limpiarForm();
        modalTitle.textContent = 'Nuevo Contacto';
        form.dataset.mode = 'crear';
    });

    document.querySelector('#tablaContactos tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.editar-contacto');
        if (btn) {
            var id = btn.getAttribute('data-id');
            fetch('/clientes/contactos/obtener/' + id)
    ...
    document.querySelector('#tablaContactos tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.eliminar-contacto');
        if (!btn) return;
        if (!confirm('¿Eliminar este contacto?')) return;
        var id = btn.getAttribute('data-id');
        fetch('/clientes/contactos/eliminar/' + id)
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/clientes/contactos/actualizar/' + form.dataset.editId : '/clientes/contactos/guardar';
    ...
    function llenarForm(data) {
        ...
        document.getElementById('f_tipo').value = data.tipo_contacto_id || '';
        document.getElementById('f_valor').value = data.valor || '';
        document.getElementById('f_contacto_nombre').value = data.contacto_nombre || '';
        document.getElementById('f_cargo').value = data.cargo || '';
