(function () {
    ...
    document.querySelector('[data-bs-target="#tarifaModal"]').addEventListener('click', function () {
        limpiarForm();
        modalTitle.textContent = 'Nueva Tarifa Anual';
    ...
    document.querySelector('#tablaTarifas tbody').addEventListener('click', function (e) {
        ...
        if (btn) {
            ...
            fetch('/tarifas/anuales/obtener/' + id)
                .then(function (r) { return r.json(); })
                .then(function (data) {
                    if (data.error) return mostrarAlerta(data.error, 'danger');
                    limpiarForm();
                    llenarForm(data);
                    modalTitle.textContent = 'Editar Tarifa Anual';
    ...
    document.querySelector('#tablaTarifas tbody').addEventListener('click', function (e) {
        ...
        if (!btn) return;
        if (!confirm('¿Eliminar esta tarifa anual?')) return;
        var id = btn.getAttribute('data-id');
        fetch('/tarifas/anuales/eliminar/' + id)
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/tarifas/anuales/actualizar/' + form.dataset.editId : '/tarifas/anuales/guardar';
