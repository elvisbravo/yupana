(function () {
    ...
    var tabla = new DataTable('#tablaDirecciones', {
        ajax: '/clientes/direcciones/listar',
        responsive: true,
        order: [[0, 'asc']],
        pageLength: 25,
    ...
    document.querySelector('[data-bs-target="#direccionModal"]').addEventListener('click', function () {
        limpiarForm();
        modalTitle.textContent = 'Nueva Dirección';
        form.dataset.mode = 'crear';
    });

    document.querySelector('#tablaDirecciones tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.editar-direccion');
        if (btn) {
            var id = btn.getAttribute('data-id');
            fetch('/clientes/direcciones/obtener/' + id)
    ...
    document.querySelector('#tablaDirecciones tbody').addEventListener('click', function (e) {
        var btn = e.target.closest('.eliminar-direccion');
        if (!btn) return;
        if (!confirm('¿Eliminar esta dirección?')) return;
        var id = btn.getAttribute('data-id');
        fetch('/clientes/direcciones/eliminar/' + id)
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var url = mode === 'editar' ? '/clientes/direcciones/actualizar/' + form.dataset.editId : '/clientes/direcciones/guardar';
    ...
    function llenarForm(data) {
        ...
        document.getElementById('f_tipo').value = data.tipo_direccion_id || '';
